...
if st.session_state.step == 7:
    ...
    case_results = evaluate_all_cases(current_key, step6_selections)

    if case_results:
        for case_title, output1, output2 in case_results:
            st.markdown(f"### {case_title}")
            st.markdown(output1, unsafe_allow_html=True)
            st.markdown(output2, unsafe_allow_html=True)
    else:
        st.warning("해당 변경사항에 대한 충족조건을 고려하였을 때, 「의약품 허가 후 제조방법 변경관리 가이드라인」에서 제시하고 있는 범위에 해당하지 않는 것으로 확인됩니다.")

    col1, col2 = st.columns(2)
    with col1:
        if st.session_state.step7_page > 0:
            if st.button("이전 항목으로"):
                st.session_state.step7_page -= 1
    with col2:
        if st.session_state.step7_page < len(targets) - 1:
            if st.button("다음 항목으로"):
                st.session_state.step7_page += 1

def evaluate_all_cases(title_key, sel):
    results = []
        ...
        cases = [
            {
                "title": "s2_2_case_1",
                "condition": (
                    sel.get("s2_2_sub_2a") == "변경 있음" and
                    sel.get("s2_2_req_3") == "충족" and
                    sel.get("s2_2_req_4") == "충족" and
                    sel.get("s2_2_req_9") == "충족" and
                    all(sel.get(k) == "미충족" for k in [
                        "s2_2_req_1", "s2_2_req_2", "s2_2_req_5",
                        "s2_2_req_6", "s2_2_req_7", "s2_2_req_8", "s2_2_req_10"
                    ])
                ),
                "output_1": """보고유형은 다음과 같습니다.  

IR, 시판전보고  
「의약품의 품목허가‧신고‧심사 규정」 제3조의2제4항 단서조항에 따른 시판전 보고(Immediate Report, IR) 수준의 변경사항입니다.""",
                "output_2": """필요서류는 다음과 같습니다.  
...
10. 변경 전·후 출발 물질 또는 중간체의 최소 1배치에 대한 시험 성적서(해당되는 경우), 출발물질 또는 중간체 변경 전·후 최종 원료의약품 2배치에 대한 배치분석 자료."""
            },
            {
                ...
                "condition": (
                    sel.get("s2_2_sub_2a") == "변경 있음" and
                    all(sel.get(f"s2_2_req_{i}") == "미충족" for i in range(1, 11))
                ),
                "output_1": """보고유형은 다음과 같습니다.  

Cmaj, 변경허가(신고)  
「의약품의 품목허가‧신고‧심사 규정」 제3조의2 및 제6조에 따라 Major change(Cmaj) 수준의 변경사항입니다.""",
                "output_2": """필요서류는 다음과 같습니다.  
...
11. 안정성 시험 이행 계획서"""
            ...
            # 나머지 s2_2 하위 case들 동일 형식으로 추가
        ]
        for case in cases:
            if case["condition"]:
                results.append((case["title"], case["output_1"], case["output_2"]))
